...
(defn get-campaign-external-id-from-req [req]
...
(defn update-campaign-handler [req]
  (let [char (get-body-parameters req)
        char-external-id (get-campaign-external-id-from-req req)]
    (validate-db-return (update-campaign char-external-id char)
                        200
                        "Campaign updated!")))

(defn delete-campaign-handler [req]
  (validate-db-return (delete-campaign (get-user-external-id req)
                                       (get-campaign-external-id-from-req req))
                      200
                      "Campaign deleted!"))

(defn get-campaign [req]
  (find-campaign (get-campaign-external-id-from-req req)))
